...
class Perspective:
    ...
    def GetEquirec(self, height, width):
        ...
        x_map = np.cos(np.radians(x)) * np.cos(np.radians(y))  # Spherical coordinate
        ...
        [R1, _] = cv2.Rodrigues(z_axis * np.radians(self.THETA))             # Get rotation matrix
        [R2, _] = cv2.Rodrigues(np.dot(R1, y_axis) * np.radians(-self.PHI))

        R1 = np.linalg.inv(R1)  # Inverse matrix
        R2 = np.linalg.inv(R2)
        ...
        xyz[:, :] = xyz[:, :]/np.repeat(xyz[:, :, 0][:, :, np.newaxis], 3, axis=2)  # Extend points from sphere to plane
